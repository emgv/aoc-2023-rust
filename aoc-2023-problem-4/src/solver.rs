...
impl Solver {
    ...
    pub fn solve_problem1(&self, min_value_for_x_and_y: f64, max_value_for_x_and_y: f64) -> Result<u64, Box<dyn Error>> {
        let lines = self.read_file(&self.input_file_problem1)?;

        let problem1_solver = problem1::Problem1::new(lines)?;
        problem1_solver.solve(min_value_for_x_and_y, max_value_for_x_and_y)
    }
